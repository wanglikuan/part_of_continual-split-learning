...
from data import get_permute_mnist, get_split_mnist, get_rotate_mnist
from SplitModelsForMnist import ResNet18, ResNet34, ResNet152, VGG, LeNet, CNN, AlexNet, ResNet101, ResNet50
from newlossoldmethod_utils_server_client import EWC, splitEWC, ewc_train, normal_train, ours_first_train, our_train, test_model
...
def ewc_process(train_loader, test_loader, labels, online=False, result_file='./ewc.txt'):
    gpu = torch.device('cuda:0')
    new_model = model_retrieval().cuda(gpu)
    models = [copy.deepcopy(new_model) for _ in range(args.num_task)]
    temp_model = copy.deepcopy(new_model)
    ...
    for task in range(args.num_task):
        ...
        if task == 0:
            ...
            for iteration in range(args.iterations):
                if online:
                    loss = ewc_train(model, labels[task], optimizer, train_loader[task], ewcs[-1:], args.lam, gpu) #与normal_train相比多了ewcs[]与args.lam
                else:
                    loss = ewc_train(model, labels[task], optimizer, train_loader[task], ewcs, args.lam, gpu) #与online区别是ewcs[-1:]为ewcs只取最后一个元素构成的列表
                print('Iteration: {}\tLoss:{}'.format(iteration, loss))
...
if __name__ == '__main__':
    ...
    if args.method == 'ewc':
        ewc_process(train_loader, test_loader, labels, result_file='./result/newlossoldmethod_{}_{}_{}.txt'.format(args.method, args.dataset, args.split))
    elif args.method == 'online':
        ewc_process(train_loader, test_loader, labels, True, result_file='./result/newlossoldmethod_{}_{}_{}.txt'.format(args.method, args.dataset, args.split))
    elif args.method == 'split_free':
        ewc_process_without_split(train_loader, test_loader, labels, False, result_file='./result/newlossoldmethod_{}_{}.txt'.format(args.method, args.dataset))
    elif args.method == 'ours':
        our_process(train_loader, test_loader, labels, result_file='./result/newlossoldmethod_alexnet_{}_{}_{}_{}_{}.txt'.format(args.FirstThreshold, args.method, args.dataset, args.split, args.threshold))
    else:
        standard_process(train_loader, test_loader, labels, result_file='./result/newlossoldmethod_{}_{}_{}.txt'.format(args.method, args.dataset, args.split))
