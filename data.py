...
def worker_labels(targets, classes, num_workers, seed=1234):
    ...
    if len(targets) < num_workers:
        classes = 1
    if classes != -1:
        temp = temp + rng.choices(list(targets), k=num_workers * classes - len(targets))
...
class NonIID_DataPartitioner(object):
    ...
    def __init__(self, train_data, test_data, sizes, classes=-1, seed=1234):
        ...
        rng = Random()
        rng.seed(seed)
        self.labels = labels = worker_labels(train_data_dict.keys(), classes, len(sizes))
        ...
        for idx, ratio in enumerate(sizes):
            ...
            for j, label in enumerate(labels[idx]):
                a, b = int(part_train_len / (len(labels[idx]) - j)), int(part_test_len / (len(labels[idx]) - j))
